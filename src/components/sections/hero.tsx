...
export default function Hero() {
  ...
  return (
    <section className="relative container mx-auto flex flex-col items-center justify-center text-center py-2 md:py-4 px-4 md:px-6 h-[80vh] overflow-hidden">
      ...
      <div className="space-y-6 max-w-3xl z-10 floating">
        <motion.h1 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          whileHover={{ scale: 1.05 }}
          transition={{ duration: 0.5, type: 'spring', stiffness: 300, damping: 15 }}
          className="text-5xl font-bold tracking-tighter sm:text-6xl md:text-7xl lg:text-8xl font-headline bg-clip-text text-transparent bg-gradient-to-r from-primary via-purple-400 to-accent relative glitch-wrapper"
          data-text="Vinish V"
          >
          <span className="glitch">Vinish V</span>
        </motion.h1>
        <motion.p 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
